...
Shader::Shader(std::string fileVertexShader, std::string fileFragmentShader) {
  LoadShader(fileVertexShader, fileFragmentShader);
}
...
void Shader::ReloadFromFile() {
  ...
  if (currentModTime > fragment_mod_time_on_load_) {
    ...
    this->LoadShader(this->vertex_file_, this->fragment_file_);
  }
}
...
void Shader::LoadShader(std::string fileVertexShader,
  ...
  // Shader s;
  this->fragment_mod_time_on_load_ = GetFileModTime(fileFragmentShader);
  this->program_id_ = program_id_;
  this->vertex_file_ = fileVertexShader;
  this->fragment_file_ = fileFragmentShader;

  // If we at any point did NOT get an error, then we say that it loaded
  // successfully
  if (!anyError) {
    std::cout << "INFO::SHADER[" << this->program_id_ << "]("
              << fileVertexShader << " + " << fileFragmentShader
              << ")::SUCCESSFULLY_LOADED" << std::endl;
  }
}
